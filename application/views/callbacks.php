        ...
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody id="main_body">
                ...
                foreach ($result as $data) {
                    ...
                    <tr style="cursor: pointer;" onclick="window.open('<?= base_url('callback-details?id='.$data->id) ?>','_blank')" id="row<?php echo $i ?>" <?php if($data->status_name=='New') echo "style='font-weight:bold'"; if(strtotime($duedate)<strtotime('today')){?> class="highlight_past" <?php }elseif(strtotime($duedate) == strtotime('today')) {?> class="highlight_now" <?php }elseif(strtotime($duedate)>strtotime('today')){ ?> class="highlight_future" <?php } ?>> 
